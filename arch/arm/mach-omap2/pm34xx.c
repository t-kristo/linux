...
static irqreturn_t _prcm_int_handle_io(int irq, void *unused)
{
	...
	c = prm_clear_mod_irqs(WKUP_MOD, 1, OMAP3430_ST_IO_MASK |
...
static irqreturn_t _prcm_int_handle_wakeup(int irq, void *unused)
{
	...
	c = prm_clear_mod_irqs(WKUP_MOD, 1, ~(OMAP3430_ST_IO_MASK |
					      OMAP3430_ST_IO_CHAIN_MASK));
	c += prm_clear_mod_irqs(CORE_MOD, 1, ~0);
	c += prm_clear_mod_irqs(OMAP3430_PER_MOD, 1, ~0);
	if (omap_rev() > OMAP3430_REV_ES1_0) {
		c += prm_clear_mod_irqs(CORE_MOD, 3, ~0);
		c += prm_clear_mod_irqs(OMAP3430ES2_USBHOST_MOD, 1, ~0);
	}
