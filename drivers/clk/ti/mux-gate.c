...
static void __init
_of_omap_mux_gate_clk_setup(struct device_node *node,
			    const struct clk_hw_omap_ops *hw_ops)
{
	...
	i = of_property_match_string(node, "reg-names", "mux-reg");
		...
		if (of_property_read_u32(node, "ti,mux-bit-shift", &val)) {
			pr_warn("%s: missing mux-bit-shift property for %s, defaulting to 0\n",
				__func__, node->name);
			val = 0;
		}
		mux->shift = val;
		if (of_property_read_u32(node, "ti,mux-bit-mask", &val)) {
			pr_err("%s: missing mux-bit-mask property for %s\n",
				__func__, node->name);
			goto cleanup;
		}
		mux->mask = val;
	...
	i = of_property_match_string(node, "reg-names", "div-reg");
		...
		if (of_property_read_u32(node, "ti,div-bit-mask", &val)) {
			pr_err("%s: missing div-bit-mask property for %s\n",
				__func__, node->name);
			goto cleanup;
		}
		div->mask = val;
		if (of_property_read_u32(node, "ti,div-bit-shift", &val)) {
			pr_warn("%s: missing div-bit-shift property for %s, defaulting to 0\n",
				__func__, node->name);
			val = 0;
		}
		div->shift = val;

		div->table = of_clk_get_div_table(node);
...
static void __init of_omap_no_wait_mux_gate_clk_setup(struct device_node *node)
{
	_of_omap_mux_gate_clk_setup(node, NULL);
}
CLK_OF_DECLARE(omap_no_wait_mux_gate_clk, "ti,no-wait-mux-gate-clock",
	       of_omap_no_wait_mux_gate_clk_setup);

static void __init of_omap_interface_mux_gate_clk_setup(struct device_node *node)
{
	_of_omap_mux_gate_clk_setup(node, &clkhwops_iclk_wait);
}
CLK_OF_DECLARE(omap_interface_mux_gate_clk, "ti,interface-mux-gate-clock",
	       of_omap_interface_mux_gate_clk_setup);

static void __init of_omap_mux_gate_clk_setup(struct device_node *node)
{
	_of_omap_mux_gate_clk_setup(node, &clkhwops_wait);
}
CLK_OF_DECLARE(omap_mux_gate_clk, "ti,mux-gate-clock",
	       of_omap_mux_gate_clk_setup);
