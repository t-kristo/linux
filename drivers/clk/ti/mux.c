 ...
 */
static int of_mux_clk_setup(struct device_node *node, struct clk_reg_ops *ops)
{
	struct clk *clk;
	...
	clk = clk_register_mux_table_reg_ops(NULL, clk_name, parent_names,
						...
						struct clk_reg_ops *ops)
{
	...
	int num_parents;
	int ret;
	struct clk_reg_def *reg_def = NULL;
	u32 val;
	...
	of_property_read_u32(node, "reg", &reg_def->offset);

	reg_def->ops = ops;

	mux->reg = reg_def;

	mux->flags |= CLK_MUX_REG_OPS;
